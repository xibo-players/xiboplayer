/**
 * DiskCache — Filesystem-backed media cache for @xiboplayer/proxy
 *
 * Stores downloaded media files on disk so they survive Service Worker updates,
 * browser cache eviction, and process restarts. Every deployment (Electron,
 * Chromium kiosk, deployed PWA server) runs the proxy, making this the single
 * durable cache layer.
 *
 * File layout:
 *   {cacheDir}/
...
/**
 * Sanitize a cache key into a safe relative path.
...
export class DiskCache {
  /**
   * @param {string} cacheDir — absolute path to cache root (e.g. ~/.local/share/xiboplayer/electron/media)
   */
  ...
  init() {
    ...
    console.log(`[DiskCache] Initialized: ${this.cacheDir}`);
  }
  ...
  _filePath(key) {
    return path.join(this.cacheDir, keyToRelative(key) + '.bin');
  }
  ...
  _metaPath(key) {
    ...
    const chunkDir = path.join(this.cacheDir, rel);
    if (fs.existsSync(chunkDir) && fs.statSync(chunkDir).isDirectory()) {
      return path.join(chunkDir, 'meta.json');
    }
    return path.join(this.cacheDir, rel + '.meta.json');
  }

  /** Absolute path for a chunk */
  _chunkPath(key, chunkIndex) {
    return path.join(this.cacheDir, keyToRelative(key), `chunk-${chunkIndex}.bin`);
  }

  /** Absolute path for chunk metadata */
  _chunkMetaPath(key) {
    return path.join(this.cacheDir, keyToRelative(key), 'meta.json');
  }
  ...
  getMetadata(key) {
    ...
    const wholeMetaPath = path.join(this.cacheDir, keyToRelative(key) + '.meta.json');
  ...
  put(key, buffer, metadata) {
    const fp = this._filePath(key);
    const metaPath = path.join(this.cacheDir, keyToRelative(key) + '.meta.json');
  ...
  delete(key) {
    ...
    // Chunk directory
    const chunkDir = path.join(this.cacheDir, keyToRelative(key));
    if (fs.existsSync(chunkDir) && fs.statSync(chunkDir).isDirectory()) {
  ...
  /**
   * Get total cache size on disk (bytes).
   * Walks the cache directory recursively.
   */
  getSize() {
    return this._dirSize(this.cacheDir);
  }
  ...
  listFiles() {
    const files = [];
    if (!fs.existsSync(this.cacheDir)) return files;

    for (const typeDir of fs.readdirSync(this.cacheDir, { withFileTypes: true })) {
